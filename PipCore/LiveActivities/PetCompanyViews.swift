...
public enum PetCompanyLook {
    public static func pose(_ state: PetCompanyAttributes.ContentState, species: PetSpecies, stale: Bool) -> (PetPose, PetProp?) {
        if stale {
            ...
            return (PetDirector.hold(scene, index: 0), state.stance.prop == .mug ? nil : state.stance.prop)
        }
    ...
    public var body: some View {
        ...
        HStack(spacing: 14) {
            ZStack(alignment: .top) {
                ...
                PetPoseView(species: identity.species, pose: pose, prop: prop, showsShadow: true)
